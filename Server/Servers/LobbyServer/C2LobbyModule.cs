...
namespace ZQ
{
    public class C2LobbyModule : IModule
    {
        ...
        public C2LobbyModule(LobbyServer server, string ip, ushort port)
        {
            m_server = server;
        ...
        public bool Init()
        {
            ...
            var mongoComponent = m_server.GetModule<MongoModule>();
            var redisComponent = m_server.GetModule<RedisModule>();
            var timerComponent = m_server.GetModule<TimerModule>();
            m_playerManager = new PlayerManager(m_server, m_service, m_messageDispatcher, mongoComponent, redisComponent, timerComponent);
        ...
        {
            if (message == null || message is not C2ZLoginZoneReq req)
            {
