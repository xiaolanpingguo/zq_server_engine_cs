...
namespace ZQ
{
    public class PlayerManager
    {
        ...
        private readonly C2SMessageDispatcher m_messageDispatcher;
        private readonly MongoComponent m_mongoComponent;
        private readonly RedisComponent m_redisComponent;
        private readonly TimerComponent m_timerComponent;

        private Dictionary<string, Player> m_players = new();
        private Dictionary<ulong, Player> m_channelPlayers = new();

        public PlayerManager(TcpService service, C2SMessageDispatcher messageDispatcher, 
            MongoComponent mongoComponent, RedisComponent redisComponent, TimerComponent timerComponent)
        {
            m_service = service;
        ...
        public async Task<C2S_ERROR_CODE> ProcessPlayerLogin(C2ZLoginZoneRes res, ulong channelId, string profileId, string ip)
        {
            ...
            if (sessionResult.Item1 != C2S_ERROR_CODE.Success)
            {
            ...
            if (session.ZoneServerId != Game.Instance.ZoneServerId)
            {
        ...
        {
            MongoComponent mongoComponent = Game.Instance.GetComponent<MongoComponent>();
