...
namespace ZQ
{
    public class S2MasterComponent : IComponent
    {
        ...
        public S2MasterComponent(string ip, ushort port)
        {
        ...
        public bool Init()
        {
            ...
            Log.Info($"master server has started, ip:{m_endPoint}");
            m_service.Start();
            return true;
        }
        ...
        private void OnZoneServersReq(ushort messageId, ulong channelId, int rpcId, object message)
        {
            ...
            {
                if (kv.Value.ServerType != (short)ServerType.ZoneServer)
                {
                    continue;
                }
